...
const (
	...
	schedulerStrategyQuotaDrain         schedulerStrategy = 4
)
...
func (s *authScheduler) pickMixedWithStrategy(ctx context.Context, providers []string, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, string, error) {
	...
	if !hasCandidate {
		return nil, "", s.mixedUnavailableForStrategyLocked(normalized, model, predicate, strategy)
	}
...
func (s *authScheduler) mixedUnavailableForStrategyLocked(providers []string, model string, predicate func(*scheduledAuth) bool, strategy schedulerStrategy) error {
	if strategy == schedulerStrategyQuotaDrain {
		if errCapacity := s.mixedQuotaDrainUnavailableErrorLocked(providers, model, predicate); errCapacity != nil {
			return errCapacity
		}
	}
	return s.mixedUnavailableErrorLocked(providers, model, predicate)
}

func (s *authScheduler) mixedQuotaDrainUnavailableErrorLocked(providers []string, model string, predicate func(*scheduledAuth) bool) error {
	now := time.Now()
	total := 0
	unavailableCount := 0
	earliest := time.Time{}
...
func (m *modelScheduler) pickReadyAtPriorityLocked(preferWebsocket bool, priority int, strategy schedulerStrategy, predicate func(*scheduledAuth) bool) *Auth {
	...
	case schedulerStrategyQuotaDrain:
...
func (m *modelScheduler) selectableCountAtPriorityLocked(preferWebsocket bool, priority int, strategy schedulerStrategy, predicate func(*scheduledAuth) bool) int {
	if strategy != schedulerStrategyQuotaDrain {
		return m.readyCountAtPriorityLocked(preferWebsocket, priority, predicate)
	}
	if m == nil {
		return 0
	}
