...
func TestCapacityStateIsExcludedFromAuthJSON(t *testing.T) {
	...
	if strings.Contains(string(payload), "capacity") || strings.Contains(string(payload), "secret-runtime-snapshot") ||
		strings.Contains(string(payload), "routing") || strings.Contains(string(payload), "secret-routing-model") {
		t.Fatalf("Auth JSON unexpectedly contains runtime capacity: %s", payload)
	}
}

func TestRecordRoutingSelectionKeepsOneActiveAccountPerProvider(t *testing.T) {
	manager := NewManager(nil, &QuotaDrainSelector{}, nil)
	...
	if !manager.RecordRoutingSelection("codex-a", "gpt-a") ||
		!manager.RecordRoutingSelection("claude-a", "claude-a") ||
		!manager.RecordRoutingSelection("codex-b", "gpt-b") {
		t.Fatal("RecordRoutingSelection() returned false")
	}

	now := time.Now()
	states := make(map[string]RoutingSelectionState)
	for _, auth := range manager.List() {
		states[auth.ID] = auth.RoutingSelection
	}
	if states["codex-a"].Active(now) {
		t.Fatal("codex-a selection remained active after codex-b was selected")
	}
	if !states["codex-b"].Active(now) || states["codex-b"].Model != "gpt-b" {
		t.Fatalf("codex-b selection = %#v, want active gpt-b", states["codex-b"])
	}
	if !states["claude-a"].Active(now) {
		t.Fatal("claude-a selection should remain active for its provider")
	}
}
...
func TestQuotaDrainSelector_UsesWeeklyWindowWhenDailyWindowTies(t *testing.T) {
	now := time.Now()
	selector := &QuotaDrainSelector{}
	daily := quotaDrainTestWindow(5, now.Add(2*time.Hour))
	daily.ID = "daily"
	weeklySoon := quotaDrainTestWindow(40, now.Add(11*time.Hour))
	weeklySoon.ID = "weekly"
	weeklyLater := quotaDrainTestWindow(40, now.Add(6*24*time.Hour))
	weeklyLater.ID = "weekly"
	auths := []*Auth{
		{ID: "weekly-later", Provider: "codex", Capacity: quotaDrainTestCapacity(now, now.Add(time.Hour), daily, weeklyLater)},
		{ID: "weekly-soon", Provider: "codex", Capacity: quotaDrainTestCapacity(now, now.Add(time.Hour), daily, weeklySoon)},
	}

	got, err := selector.Pick(context.Background(), "codex", "gpt", cliproxyexecutor.Options{}, auths)
	if err != nil {
		t.Fatalf("Pick() error = %v", err)
	}
	if got == nil || got.ID != "weekly-soon" {
		t.Fatalf("Pick() auth = %#v, want weekly-soon", got)
	}
}

func TestSchedulerQuotaDrain_UsesEveryWindowForRanking(t *testing.T) {
	now := time.Now()
	daily := quotaDrainTestWindow(5, now.Add(2*time.Hour))
	daily.ID = "daily"
	weeklySoon := quotaDrainTestWindow(40, now.Add(11*time.Hour))
	...
	scheduler := newSchedulerForTest(
		&QuotaDrainSelector{},
		&Auth{ID: "weekly-later", Provider: "codex", Capacity: quotaDrainTestCapacity(now, now.Add(time.Hour), daily, weeklyLater)},
		&Auth{ID: "weekly-soon", Provider: "codex", Capacity: quotaDrainTestCapacity(now, now.Add(time.Hour), daily, weeklySoon)},
	)
	...
	if got == nil || got.ID != "weekly-soon" {
		t.Fatalf("pickSingle() auth = %#v, want weekly-soon", got)
	}
}
