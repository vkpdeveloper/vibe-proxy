    ...
    <div className={styles.page}>
      <header className={styles.header}>
        <div>
          <span className={styles.eyebrow}>
            {t('usage.eyebrow', { defaultValue: 'COST INTELLIGENCE' })}
          </span>
          <h1>{t('usage.title', { defaultValue: 'Usage & spend' })}</h1>
          <p>
            {t('usage.subtitle', {
              defaultValue:
                'Persistent token accounting across every account, provider, and model.',
            })}
          </p>
        </div>
      ...
      <section className={styles.summaryGrid} aria-busy={loading}>
        ...
        <article className={styles.summaryCard}>
          ...
          <span className={styles.metricHint}>
            {report
              ? `${report.totals.successful.toLocaleString()} successful · ${report.totals.failed.toLocaleString()} failed`
              : '—'}
          </span>
        </article>
        <article className={styles.summaryCard}>
          ...
          {daily.length === 0 ? (
            ...
            daily.map((day) => (
              <div
                ...
                key={day.date}
                title={`${day.date}: ${formatMoney(day.estimated_usd, currency)}`}
              >
                <span className={styles.barValue}>{formatMoney(day.estimated_usd, currency)}</span>
                <div
                  className={styles.bar}
                  style={{
                    height: `${Math.max(4, (day.estimated_usd / Math.max(maxDailyCost, 0.000001)) * 100)}%`,
                  }}
                />
      ...
      <section className={styles.breakdownCard}>
        <div className={styles.sectionHeader}>
          ...
          <div className={styles.dimensionTabs} role="tablist">
            {(['client', 'provider', 'account', 'model', 'auth'] as Dimension[]).map((value) => (
              <button
                type="button"
                role="tab"
      ...
      <section className={styles.notesGrid}>
        <article className={styles.noteCard}>
          ...
          <div>
            ...
            <p>
              It is the public API equivalent of the observed tokens, including cache and
              long-context rates. OAuth subscriptions and negotiated discounts may make the amount
              on your invoice different.
            </p>
          </div>
        </article>
        <article className={styles.noteCard}>
      ...
      {unpricedModels.length > 0 && (
        <section className={styles.unpricedCard}>
          <div className={styles.sectionHeader}>
            <div>
              <h2>Models needing a price</h2>
              <p>These tokens remain in totals but are excluded from the dollar estimate.</p>
            </div>
          </div>
...
export function UsagePageError() {
  ...
  return (
    <section className={styles.fatalState} role="alert">
      <div className={styles.fatalIcon}>
