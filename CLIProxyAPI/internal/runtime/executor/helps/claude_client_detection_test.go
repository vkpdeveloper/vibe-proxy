...
func TestDetectClaudeCodeRequestRejectsMalformedNativeSignals(t *testing.T) {
	tests := []struct {
		...
		{name: "previous baseline user agent", headers: http.Header{"User-Agent": {"claude-cli/2.1.258 (external, cli)"}, "X-App": {"cli"}, "Anthropic-Beta": {"claude-code-20250219"}}, userID: validClaudeCodeMetadataUserID},
